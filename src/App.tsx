...
function App() {
  ...
  const [mtSupportLevel, setMtSupportLevel] = useState<'HIGH' | 'LOW'>('HIGH');
  const [mtEfCapacity, setMtEfCapacity] = useState<NeuroMetrics['ef_capacity']>('moderate');
  ...
  const sendMessage = useCallback(
    async (userText: string, opts?: { showUser?: boolean; updateContext?: boolean }) => {
      ...
      try {
        const finalized = await streamProfileQuestion(
          ...
          lastAssistantQuestionRef.current
            ? [...context, { question: lastAssistantQuestionRef.current, answer: userText }]
            : context,
          async (word) => {
        ...
        if (lastAssistantQuestionRef.current) {
          const parsed = parseQuestionAndOptions(lastAssistantQuestionRef.current);
          if (parsed.options.length > 0) {
            setOptions(parsed.options);
            // Optionally remove the raw options from the assistant bubble for a cleaner look
            setMessages((prev) =>
              prev.map((m) =>
                m.id === assistantId ? { ...m, content: parsed.question || m.content } : m,
              ),
            );
          }
        }
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: `error-${Date.now()}`,
            role: 'system',
            content: 'Something went wrong while talking to the questionnaire API.',
          },
        ]);
      } finally {
  ...
  const handleFileChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file.size > 20 * 1024 * 1024) {
        setMtError('File too large. Please upload a PDF under 20 MB.');
        setMtFileName(null);
        setMtFileBase64(null);
        return;
      }
      ...
      setMtFileMime(file.type || 'application/pdf');
  ...
  const handleGenerateMicrotasks = useCallback(async () => {
    if (!mtDescription.trim()) {
      setMtError('Please paste an assignment description first.');
      return;
    }
            ...
            <div className="flex h-full items-center justify-center py-8">
              <div className="text-center space-y-4 max-w-md">
                ...
                <p className="text-sm text-slate-500">
                  We&apos;ll ask a few quick questions about how you start, focus, process
                  instructions, and plan. Your answers tune the microtask engine to your neuro
                  profile.
                </p>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                  ...
                  <label className="flex flex-col gap-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setMtSupportLevel(e.target.value === 'LOW' ? 'LOW' : 'HIGH')
                      }
                      className="rounded-md border border-slate-300 bg-white px-2 py-1.5 text-xs outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30"
                    >
                      <option value="HIGH">HIGH – more scaffolding</option>
                      <option value="LOW">LOW – fewer, larger chunks</option>
                    </select>
                  </label>
                </div>
                ...
                <div className="flex items-center justify-between gap-2 text-[0.7rem] pt-1">
                  <label className="flex items-center gap-2">
                    <span className="text-slate-500 whitespace-nowrap">PDF attachment</span>
                    <input
                      type="file"
                ...
                {mtFileName && (
                  <div className="flex items-center justify-between gap-2 text-[0.7rem] text-slate-500">
                    <span className="truncate">
                      Attached: <span className="font-medium text-slate-700">{mtFileName}</span>
                    </span>
                    <button
                      type="button"
                      onClick={() => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
                  <label className="flex flex-col gap-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setMtEfCapacity(
                          (e.target.value as NeuroMetrics['ef_capacity']) || 'moderate',
                        )
                      }
                      className="rounded-md border border-slate-300 bg-white px-2 py-1.5 text-xs outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30"
                    >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setMtProcessingStyle(
                          (e.target.value as NeuroMetrics['processing_style']) || 'standard',
                        )
                      }
                      className="rounded-md border border-slate-300 bg-white px-2 py-1.5 text-xs outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30"
                    >
                  ...
                  <label className="flex flex-col gap-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setMtCoachTone(
                          (e.target.value as NeuroMetrics['coach_tone']) || 'reassuring',
                        )
                      }
                      className="rounded-md border border-slate-300 bg-white px-2 py-1.5 text-xs outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30"
                    >
        ...
        <footer className="px-4 py-3 border-t border-slate-200 bg-slate-50">
          {activeTab === 'calibration' ? (
            ...
            <div className="text-[0.7rem] text-slate-400">
              Powered by the Margati microtasks engine · This sandbox uses a demo course/assignment
              id and does not write to Canvas.
            </div>
          )}
        </footer>
      </div>
